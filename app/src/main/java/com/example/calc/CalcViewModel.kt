...
class CalcViewModel : ViewModel(), CalculatorInteractionListener {
    ...
    override fun onOperatorClick(operation: Operation) {
        if (calculationStack.isEmpty()) {
            _state.update {
                it.copy(
                    currentLine = INVALID_INPUT.toString()
                )
            }
            return
        }
    ...
    override fun onToggleSignClick() {
    ...
    override fun onEqualClick() {
        if (isValidStack()) {
            ...
            if (result != ERROR.toString() && result.isNotEmpty() && result != "0") {
    ...
    private fun evaluateExpression(expression: String): String {
        ...
        for (i in parts.indices) {
            if (i % 2 == 0) {
                numbers.add(parts[i].toFloatOrNull() ?: return ERROR.toString())
            } else {
        ...
        while (i < operations.size) {
            ...
            if (op == "x" || op == "/" || op == "%") {
                ...
                val result = when (op) {
                    "x" -> left * right
                    "/" -> {
                        if (right == 0.0f) return ERROR_DIVIDE_BY_ZERO.toString()
                        left / right
                    }
                    "%" -> {
                        if (right == 0.0f) return ERROR_DIVIDE_BY_ZERO.toString()
                    ...
                    else -> return ERROR_DIVIDE_BY_ZERO.toString()
                }
    ...
    private fun updateCurrentLine() {
        _state.update {
            it.copy(
                currentLine = if (calculationStack.isEmpty()) "0"
                else calculationStack.joinToString("")
            )
        }
    }
    ...
    companion object {
        ...
        const val ERROR = "some thing went wrong"
    }
}